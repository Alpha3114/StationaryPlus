...
function actionBadgeColor(string $action): array {
    if (str_contains($action, 'CREATE'))   return ['var(--success)', 'rgba(76,175,80,0.1)'];
    if (str_contains($action, 'REJECT') || str_contains($action, 'DELETE')) return ['var(--danger)', 'rgba(239,68,68,0.1)'];
    if (str_contains($action, 'STATUS') || str_contains($action, 'TOGGLE')) return ['#e65100', 'rgba(244,162,97,0.15)'];
    return ['var(--primary)', 'var(--primary-tint-medium)'];
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',system-ui,sans-serif; }
        ...
        .filter-bar { padding:16px 20px;border-bottom:1px solid var(--border);background:var(--primary-tint-subtle);display:flex;gap:10px;flex-wrap:wrap;align-items:center; }
        ...
        .result-count { font-size:12px;color:var(--text-secondary);background:var(--primary-tint-light);padding:3px 10px;border-radius:20px;font-weight:600;white-space:nowrap; }
        .audit-table { width:100%;border-collapse:collapse; }
        .audit-table thead { background:var(--primary-tint-subtle);border-bottom:2px solid var(--border); }
        ...
        .audit-table tbody tr:hover { background:var(--primary-tint-subtle); }
        ...
        .page-link.active { background:var(--primary);color:var(--on-primary);border-color:var(--primary); }
